...
def read_mlcc_2nd_subset(root_dir, binary=True, mode='train', sample_size=None):
    """
    Load the MLCC-2nd data subset from disk.
    ...
    if mode == 'total':
        set_filename_list = []
        #for set_txt_filename in ['train.txt', 'test.txt']:
        for set_txt_filename in ['train1.txt', 'validation1.txt', 'test.txt']:
            set_filename_path = os.path.join(root_dir, 'imageset', 'imageset1', set_txt_filename)    # FIXME
            with open(set_filename_path, 'r') as fid:
                set_filename_list += fid.read().split('\n')[:-1]
        set_size = len(set_filename_list)
    else:    # mode == 'train' or mode == 'test' or mode == 'validation'
        #set_txt_filename = 'train.txt' if mode == 'train' else 'test.txt'
        if mode == 'train':
            set_txt_filename = 'train1.txt'
        elif mode == 'validation':
            set_txt_filename = 'validation1.txt'
        else:
            set_txt_filename = 'test.txt'
        set_filename_path = os.path.join(root_dir, 'imageset', 'imageset1', set_txt_filename)    # FIXME
        with open(set_filename_path, 'r') as fid:
            set_filename_list = fid.read().split('\n')[:-1]
        set_size = len(set_filename_list)
