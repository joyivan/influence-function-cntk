...
set_trace()
method = input('type method(hard or soft or etc): ')
...
for fold_idx in range(1,6):
    ...
    for beta in [0.5, 0.6, 0.7, 0.8, 0.9, 0.95, 0.99]:
        print('beta:', beta)
        ...
        optimizer = Optimizer(model, train_set, evaluator, val_set=val_set,
                fold_idx=fold_idx, trainval_ratio=beta, **hp_d)
        ...
        test_result_dict[fold_idx][beta] = test_score

        print('intermediate test_result_dict value:\n{}'.format(test_result_dict))
        with open('./output/intermediate_cv_emnist_{}_test_result.pkl'.format('stop_'*hp_d['stop']+method), 'wb') as fid:
            pkl.dump(test_result_dict, fid)

with open('./output/cv_emnist_{}_test_result.pkl'.format('stop_'*hp_d['stop']+method), 'wb') as fid:
    pkl.dump(test_result_dict, fid)
print('Done.')
