...
#file_dir = './compare/result_net_nn_if_nn/train_F_50837'
file_dir = './compare/result_net_nn_if_nn/train_O_111780'
...
for fn in ['if_cg_logreg', 'if_se_logreg', 'if_v_cos', 'if_v']:
    ...
    for i in range(num_sample):
        ...
        images[-num_sample+i-1][1] = 1  # botk to green - adv
        images[-2*num_sample+i-1][2] = 1 # topk to blue - disad
    
    emb_image = view_tsne_embeddings(tsne_embeddings, images, test_labels)
    emb_dir = './images/tsne/'+file_dir.split('/')[-1]
    if not os.path.exists(emb_dir):
        os.makedirs(emb_dir)
    imsave(emb_dir+'/convnet_embed_'+fn+'.jpg', emb_image)
